...
impl<'src> Lexer<'src> {
    ...
    fn skip_whitespace_on_line(&mut self) {
        while let Some(&(Ok(ref tok), _)) = self.peek_raw() {
            ...
            // Logos already skips spaces, so we just continue
            break;
        }
    }
    ...
    #[allow(dead_code)]
    fn read_frontmatter_line(&mut self, _start: usize) -> Option<SpannedToken> {
        // Read until newline
        let line_start = self.pos;
        let mut line_end = line_start;

        while let Some((tok_res, span)) = self.next_raw() {
            line_end = span.end;
            if matches!(tok_res, Ok(RawToken::Newline)) {
                break;
            }
        }

        let text = self.source[line_start..line_end].trim();
        if text.is_empty() || text == "\n" {
            return None;
        }

        Some(SpannedToken {
            token: Token::Text(SmolStr::new(text.trim_end_matches('\n'))),
            span: line_start..line_end,
        })
    }

    #[allow(dead_code)]
    fn read_prose_line(&mut self) -> Option<SmolStr> {
        let start = self.pos;
        let mut end = start;
    ...
    #[allow(dead_code)]
    fn is_prose_line_start(&self) -> bool {
        ...
        // Not prose if it starts with special markers
...
mod tests {
    ...
    #[test]
    fn test_effect() {
        let source = "~ credits += 10";
        let tokens = lex(source);
        assert!(matches!(tokens[0].token, Token::EffectMarker));
        assert!(matches!(&tokens[1].token, Token::Identifier(s) if s == "credits"));
