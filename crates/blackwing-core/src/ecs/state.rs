...
use crate::{ActorId, EntityId, FlagId, GameTime, ResourceId, Rng, Value};
use indexmap::IndexMap;
use rustc_hash::FxHashMap;
use serde::{Deserialize, Serialize};

use super::{ChronicleEntry, Entity, EntityKey, EntityStorage, Scope, ScopeKind, ScopeStack, WorldEvent};
...
impl WorldState {
    ...
    pub fn apply_event(&mut self, event: &WorldEvent) {
        match event {
            ...
            // UI events don't modify world state - they're for frontend
            WorldEvent::UiSet { .. }
            | WorldEvent::UiUpdated { .. }
            | WorldEvent::ModalOpened { .. }
            | WorldEvent::ModalClosed
            | WorldEvent::UiCleared => {
                // No state change needed
            }

            // === Spatial events ===
            WorldEvent::PositionSet { entity, x, y } => {
                self.entities
                    .set_component(*entity, "position_x", Value::Float((*x).into()));
                self.entities
                    .set_component(*entity, "position_y", Value::Float((*y).into()));
            }
            ...
            WorldEvent::EntityMoved {
                ...
                self.entities
                    .set_component(*entity, "position_x", Value::Float((*to_x).into()));
                self.entities
                    .set_component(*entity, "position_y", Value::Float((*to_y).into()));
            }

            WorldEvent::RoomChanged { .. } => {
                // Room state is tracked separately (not in WorldState)
                // This event is for the runtime/renderer to handle
            }

            // === Combat events ===
            // Combat events track what happened but state is already modified by the runtime
            WorldEvent::DamageTaken {
                ...
                self.entities
                    .set_component(*entity, "health", Value::Int(*health_remaining));
            }

            WorldEvent::EntityDied { .. } => {
                // Death handling (despawn, etc.) is done by the runtime
                // This event is for observers/UI
            }
            ...
            WorldEvent::Healed {
                ...
                self.entities
                    .set_component(*entity, "health", Value::Int(*health_now));
            }

            WorldEvent::InvincibilitySet {
                entity,
                duration_ms,
            } => {
                self.entities
                    .set_component(*entity, "invincible", Value::Bool(true));
                self.entities
                    .set_component(*entity, "invincible_timer", Value::Int(*duration_ms as i64));
            }
        }
    }
