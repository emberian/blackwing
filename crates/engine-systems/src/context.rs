...
impl<'a> WorldView<'a> {
    ...
    pub fn entities(&self) -> &engine_world::EntityStorage {
        &self.state.entities
    }

    /// Get the current game time
    pub fn time(&self) -> &engine_primitives::GameTime {
        &self.state.time
    }

    /// Check a flag value
    pub fn flag(&self, flag: &str) -> Option<&engine_primitives::Value> {
        self.state.flags.get(&flag.into())
    }

    /// Get a resource value
    pub fn resource(&self, resource: &str) -> i64 {
        let key: engine_primitives::ResourceId = resource.into();
        self.state.resources.get(&key).copied().unwrap_or(0)
    }

    /// Get an actor's scope stack
    pub fn scopes(&self, actor: &EntityId) -> Option<&engine_world::ScopeStack> {
